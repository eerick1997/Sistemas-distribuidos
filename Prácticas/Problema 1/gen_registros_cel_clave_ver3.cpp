...
{
	char telefono[11], curp[19], t[11], sexo, fin[1];
	...
	for (j = 0; j < n; j++)
	{
		...
		string cadena = reg1.celular;
		cadena += reg1.CURP;
		cadena += reg1.partido;
		write(destino, cadena.c_str(), cadena.length());
		if (j < n - 1)
		{
			write(destino, fin, sizeof(fin));
		}
		// write(1, cadena.c_str(), cadena.length());
